...
public class MapRepresentation implements Serializable {
	...
	public boolean equals(SerializableSimpleGraph<String, MapAttribute> other) {
		if (other == null)
			return false;

        SerializableSimpleGraph<String, MapAttribute> currentMap = this.getSerializableGraph();

        // Comparer les nœuds
        if (!currentMap.getAllNodes().equals(other.getAllNodes())) {
            return false;
        }

        // Comparer les arêtes
        for (SerializableNode<String, MapAttribute> node : currentMap.getAllNodes()) {
            Set<String> edges = currentMap.getEdges(node.getNodeId());
            Set<String> otherEdges = other.getEdges(node.getNodeId());

            if (edges == null && otherEdges == null) {
                continue;
            }

            if (edges == null || otherEdges == null || !edges.equals(otherEdges)) {
                return false;
            }
        }
